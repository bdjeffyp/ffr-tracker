// Collection of items
items = {
  bridge: { val: itemState.off, max: itemState.on },
  lute: { val: itemState.off, max: itemState.on },
  crown: { val: itemState.off, max: itemState.on },
  crystal: { val: itemState.off, max: itemState.on },
  herb: { val: itemState.off, max: itemState.on },

  ship: { val: itemState.off, max: itemState.on },
  key: { val: itemState.off, max: itemState.on },
  tnt: { val: itemState.off, max: itemState.on },
  ruby: { val: itemState.off, max: itemState.on },
  rod: { val: itemState.off, max: itemState.on },

  canal: { val: itemState.off, max: itemState.on },
  floater: { val: itemState.off, max: itemState.on },
  tail: { val: itemState.off, max: itemState.on },
  bottle: { val: itemState.off, max: itemState.on },
  oxyale: { val: itemState.off, max: itemState.on },

  canoe: { val: itemState.off, max: itemState.on },
  slab: { val: itemState.off, max: itemState.on },
  chime: { val: itemState.off, max: itemState.on },
  cube: { val: itemState.off, max: itemState.on },
  adamant: { val: itemState.off, max: itemState.on },

  earth: { val: itemState.off, max: itemState.on },
  fire: { val: itemState.off, max: itemState.on },
  water: { val: itemState.off, max: itemState.on },
  air: { val: itemState.off, max: itemState.on },
...
// NPCs
npcs = {
  king: { val: itemState.off, max: itemState.on },
  sara: { val: itemState.off, max: itemState.on },
  bikke: { val: itemState.off, max: itemState.on },
  sarda: { val: itemState.off, max: itemState.on },
  sages: { val: itemState.off, max: itemState.on },
  robot: { val: itemState.off, max: itemState.on },

  astos: { val: itemState.off, max: itemState.on },
  matoya: { val: itemState.off, max: itemState.on },
  elf: { val: itemState.off, max: itemState.on },
  nerrick: { val: itemState.off, max: itemState.on },
  smyth: { val: itemState.off, max: itemState.on },
  lufeinian: { val: itemState.off, max: itemState.on },

  shopkeeper: { val: itemState.off, max: itemState.on },
  fairy: { val: itemState.off, max: itemState.on }
};
...
// TODO: Add overworld map with incentivized chest locations to mark off later
map = {
  populate: function () {
    // Place all chest icons on the map
    ...
    // $(".chest, .keyShop, .takeAny").hover(function () {
    //   // hover on
    ...
    // $(".icon, #timer").on("contextmenu", function () {
    //   return false;
    // });
...
// Removes the previous class state and adds the new class state
function toggleIcon(iconId, collection) {
  $("#" + iconId)
  .attr("class", function (i, c) { return c.replace(/(^|\s)state\S+/g, ''); })
  .addClass("state" + collection[iconId].val);
}
...
toggle = {
  icon: function (icon, reverse = false) {
    ...
    // Update the icon with the new class
    toggleIcon(icon.id, items);
    logic.apply();
  },
  npc: function (icon) {
    // Update the state value
    curVal = npcs[icon.id].val;
    npcs[icon.id].val = (curVal == 0) ? npcs[icon.id].max : (curVal - 1);

    // Update the icon with the new class
    toggleIcon(icon.id, npcs);
  }
};
